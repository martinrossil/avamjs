...
import TrailersGenresScreen from "./TrailersGenresScreen.js";
import TrailersCountriesScreen from "./TrailersCountriesScreen.js";
...
export default class TrailersFilterScreen extends BaseScreen
{
    constructor()
    {
        super();
    ...
    get filterListDataProvider()
    {
        ...
        {
            let filters = [ { icon : IconNames.MOVIE_FILTER, l : Strings.GENRES, h : Paths.TRAILERS_GENRES }, 
                            { icon : IconNames.PUBLIC, l : Strings.COUNTRIES, h : Paths.TRAILERS_COUNTRIES } 
                          ];    
    ...
    get screensNavigator()
    {
        if( !this._screensNavigator )
        {
            ...
            this._screensNavigator.uid = UIDS.TRAILERS_FILTERS_SCREEN_NAVIGATOR;
            this._screensNavigator.layoutData = new AnchorLayoutData( 8, 152, 8, 0 );
            this._screensNavigator.addScreen( new TrailersGenresScreen() );
            this._screensNavigator.addScreen( new TrailersCountriesScreen() );
        }
        return this._screensNavigator;
    }
}
customElements.define( "trailers-filter-screen", TrailersFilterScreen );